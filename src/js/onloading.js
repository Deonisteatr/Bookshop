let onloading = () => {
    // переменные
const apiKey = 'AIzaSyCJTLj6-BioGtky20qNn8z3aKu-HIe44H4';
let activeTheme = document.querySelector('.gallery-cards__category-item').getAttribute('data-category');
let currentStep = 0;
...
const getAndShow = async() => {
    ...
    books.forEach((book) => {
        ...
        // отрисовка книги
        let newBook = `<div class="gallery-cards__book" id="${book.id}">
            <img src="${img}" alt="${book.title}" class="gallery-cards__book-img">
        ...
        document.querySelector('.gallery-cards__books').innerHTML += newBook;
    ...
    const container = document.querySelector(".numb");
...
document.addEventListener('click', (event) => {
    if (event.target.classList.contains('gallery-cards__category-item')) {
        currentStep = 0;
        event.preventDefault();
        document.querySelector('.gallery-cards__category-item.active')?.classList?.remove('active');
        event.target.classList.add('active');
        document.querySelector('.gallery-cards__books').innerHTML='';
